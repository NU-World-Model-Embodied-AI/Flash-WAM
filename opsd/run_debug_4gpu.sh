#!/bin/bash
# Four-GPU debug launcher for Flash-WAM OPSD stage-2 training.
# Defaults:
...
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/opsd/debug_output_4gpu}"

export OPSD_VIDEO_STEPS="${OPSD_VIDEO_STEPS:-1}"
...
export LOAD_WORKER="${LOAD_WORKER:-2}"
...
if not isinstance(tasks, list) or len(tasks) != 12 or len(set(tasks)) != 12:
    raise SystemExit("OPSD_TASK_LIST must be a JSON array of 12 unique task names.")
if not all(isinstance(task, str) for task in tasks):
    raise SystemExit("OPSD_TASK_LIST entries must be strings.")
PY
...
echo "Flash-WAM OPSD 4-GPU debug launch"
