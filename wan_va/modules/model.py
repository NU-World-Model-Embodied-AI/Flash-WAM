...
class FlexAttnFunc(nn.Module):
    ...
    def init_mask(
        ...
        # Standard: action of chunk c gets frame_id 2c+1 (one *after* the video
        # of that chunk) so a noisy video token (frame_id 2c) cannot attend the
        # current chunk's action under the strictly-causal noise->clean rule
        # (kv<q) — this is L_dyn (video continues from past clean video+actions).
        # FDM (Eq 13): shift action to 2c-1 so the noisy video of chunk c CAN
        # attend the *clean* action of chunk c — forward-dynamics conditioning
        # v(z_{t+1} | z_<=t, a_<=t). Standard pass (fdm=False) is unchanged.
        action_frame_offset = -1 if fdm else 1
        frame_ids = torch.cat([latent_frame_id // chunk_size * 2] * 2 + [action_frame_id // chunk_size * 2 + action_frame_offset] * 2)
    ...
    def __init__(
        ...
        if attn_mode == 'torch':
            ...
            if flash_attn_func is None:
                raise ImportError(
                    "attn_mode='flashattn' requires flash_attn_func from "
                    "flash_attn_interface or flash_attn."
                )
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward_train(self, input_dict, fdm=False):
    ...
    def forward(
        ...
        """
        if train_mode:
            return self.forward_train(input_dict, fdm=fdm)
